...
\newacronym{ids}{IDS}{\Gls{intrusion} Detection System}
\newacronym{apt}{APT}{Advanced Persistant Threat}
\newacronym{fim}{FIM}{File Integrity Monitoring} 
\newacronym{fids}{FIDS}{Forensics-based \Gls{intrusion} Detection System} 
\newacronym{hids}{HIDS}{Host-based \Gls{intrusion} Detection System} 
\newacronym{nids}{NIDS}{Network-based \Gls{intrusion} Detection System} 
...
\newglossaryentry{aide}{name=aide, description={\Gls{opensource} \gls{hids} using \gls{fim} to detect \glspl{intrusion}}}
...
\newglossaryentry{samhain}{name=samhain, description={\Gls{opensource} \gls{hids} which mainly uses kernel hook to find \glspl{intrusion} and contains extensive centralization support for configuration and alerting}}
...
\newglossaryentry{yyaml}{name=YAML, description={YAML is a human friendly data serialization standard for all programming languages.}}
...
name=anomaly, 
plural=anomalies,
description={An unexpected change on a system. For a change to be unexpected it needs to be covered by the configuration changes are unexpected only if the configuration says so. Some examples that might cause anomalies on the \gls{fs}, changed permissions, new files, deleted files}}
...
description={A deterministic one way function that fullfills collision resistance and other cryptographic properties. Implementations are for instance the SHA-2 and SHA-3 families}}
\newglossaryentry{nonhash}{
name=non-cryptographic hash function, 
plural=non-cryptographic hash functions,
description={A deterministic one way function that does not fullfill the hard to achieve properties that make a \gls{hash}. Often used when speed is more important than collision resistance}}
\newglossaryentry{fs}{
name=file system, 
description={A file system is used to create a layer of abstraction between the blocks of the storage medium and the operating system. There are multiple file system types which are in use with different capabilities. Additionally to the files the file systems keep track of meta data to each file. This meta data includes some timestamps (created, last accessed, etc) and more}}
\newglossaryentry{pytsk}{name=pytsk, description={Python bindings for \gls{tsk}}}
